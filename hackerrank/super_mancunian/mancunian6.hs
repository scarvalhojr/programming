...
import qualified Data.List                  as L (sortOn, unfoldr)
import qualified Data.Maybe                 as M (isNothing, fromJust)
import qualified Data.IntMap                as H (IntMap, empty, insert, toList,
                                                  findWithDefault)
...
main = do
    ...
    let numlevels = head numbers
    let conns = map createConnection (S.chunksOf 3 (drop 2 numbers))
    let (cost, count) = case magicPaths numlevels conns of
                          Nothing     -> (-1, -1)
                          Just (c, n) -> (c, n)
...
createConnection (l1:l2:cost:[]) = Connection l1 l2 cost
createConnection _ = error "Invalid input"
