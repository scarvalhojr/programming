...
import qualified Data.Vector                as V (Vector, unfoldr, zip, zipWith,
                                                  map, toList)
...
parseIntList :: [Char] -> V.Vector Int
parseIntList cs = V.unfoldr step (B.pack cs)
  where step bs = case B.readInt bs of
                    Nothing     -> Nothing
                    Just (k, t) -> Just (k, B.drop 1 t)

buildOrderedTownList :: V.Vector Int -> V.Vector Int -> OrderedTowns
buildOrderedTownList pos = L.sort . V.toList . V.map (uncurry Town) . V.zip pos

buildOrderedCloudList :: V.Vector Int -> V.Vector Int -> OrderedClouds
buildOrderedCloudList pos = L.sort . V.toList . V.map (uncurry Cloud) . V.zipWith convert pos
