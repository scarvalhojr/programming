...
/*
 * Declara um ponteiro para um vetor de ponteiros
 * que apontam para os blocos
 */
struct blocodef *bloco;

/*
 * Declara um ponteiro para um vetor de ponteiros
 * que apontam para os primeiros blocos de cada fila
 */
struct blocodef **fila;
...
int iniciar()
{
	bloco = (blocodef *) malloc(dimensao * sizeof (blocodef));
	if (!bloco) return 0;

	fila = (blocodef **) malloc(dimensao * sizeof (blocodef *));
	if (!fila) return 0;
...
void main()
{
	...
	// Aloca as estruturas de memória
	if (!iniciar()) return;
	...
	// Libera as estruturas de memória
	finalizar();
}
